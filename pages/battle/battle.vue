  ...
  <view class="page">
    ...
    <view v-if="!snap" class="lobby">
      ...
      <view class="mini-tip">
        ...
        <br />🔒 <b>词性本局固定</b>：一张牌的词性开局就定好、全程不变（多词性词只取第一个词性）。
        <br />⚠️ <b>攻击手阵亡就输</b>：一方场上的动词（攻击手）全部阵亡 → 该方无法再造成伤害，直接判负。所以务必保护好你的动词！
        <br />💰 <b>金币场</b>：显示英文单词、凭记忆拼写它的中文意思（同「拼中文」）；真人实时对战，赢家拿走双方押注。
      </view>
    </view>
...
export default {
  ...
  data() {
    return {
      ...
      // 💰 金币场：每档位所需单词数（后端为准，这里做兜底显示）词数 = 金币数
      betTroopMap: { 10: 10, 20: 20, 50: 50, 100: 100 },
  ...
  methods: {
    ...
    async onStart() {
      ...
      try {
        ...
        this.starting = false;
        // 服务端已给出明确原因（例如：队伍里没有动词/没有健康单词）优先展示
        this.msg = (e && (e.msg || e.message)) || '无法开始：可能你还没有健康的可出战单词';
      }
    },
