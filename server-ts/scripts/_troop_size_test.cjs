...
(async () => {
  ...
  ok(BL.goldTroopSize(10) === 3, 'goldTroopSize(10) = 3');
  ok(BL.goldTroopSize(30) === 4, 'goldTroopSize(30) = 4');
  ok(BL.goldTroopSize(50) === 5, 'goldTroopSize(50) = 5');
  ok(BL.goldTroopSize(100) === 7, 'goldTroopSize(100) = 7');
  ok(BL.goldTroopSize(200) === 10, 'goldTroopSize(200) = 10');
  ok(BL.goldTroopSize(999) === BL.TROOP_SIZE, '未知档位回退 TROOP_SIZE');
  ok(BL.MAX_TROOP_SIZE === 10, 'MAX_TROOP_SIZE = 10');
  ...
  if (token) {
    ...
    ok(Array.isArray(d.options) && d.options.length === 5, 'options 5 档');
    ok(Array.isArray(d.optionsDetail) && d.optionsDetail.length === 5, 'optionsDetail 5 档');
    ...
    if (Array.isArray(d.optionsDetail)) {
      ...
      ok(d.optionsDetail[0].troop === 3 && d.optionsDetail[4].troop === 10, '档位映射正确(3..10)');
    }
  }
